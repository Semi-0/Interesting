...
import { LNumber, LSymbol, List, LBoolean } from '../definition/LispElement';
...
describe('Interpreter Tests', () => {
  ...
  test('evaluate lambda expression with one variable', () => {
    ...
    const lambdaExpr = new List([
      new LSymbol('lambda'),
      new List([new LSymbol('x')]), // Parameter
      new LSymbol('x') // Body: return x
    ]);

    // Call expression that applies the lambda to argument 5
    const callExpr = new List([lambdaExpr, new LNumber(5)]);
  ...
  test('evaluate lambda expression with increment operation', () => {
    ...
    const lambdaExpr = new List([
      new LSymbol('lambda'),
      new List([new LSymbol('x')]), // Parameter
      new List([new LSymbol('+'), new LSymbol('x'), new LNumber(1)]) // Body: x + 1
    ]);

    // Call expression that applies the lambda to argument 1
    const callExpr = new List([lambdaExpr, new LNumber(1)]);
    const result = evaluate(callExpr, env);
  ...
  test('evaluate lambda expression with two variables', () => {
    ...
    const lambdaExpr = new List([
      new LSymbol('lambda'),
      new List([new LSymbol('x'), new LSymbol('y')]), // Parameters
      new List([new LSymbol('+'), new LSymbol('x'), new LSymbol('y')]) // Body: x + y
    ]);

    // Call expression that applies the lambda to arguments 5 and 3
    const callExpr = new List([lambdaExpr, new LNumber(5), new LNumber(3)]);
    const result = evaluate(callExpr, env);
  ...
  test('evaluate lambda expression with three variables', () => {
    ...
    const lambdaMultExpr = new List([
      new LSymbol('lambda'),
      new List([new LSymbol('a'), new LSymbol('b'), new LSymbol('c')]), // Parameters
      new List([new LSymbol('*'), new LSymbol('a'), new LSymbol('b'), new LSymbol('c')]) // Body: a * b * c
    ]);

    // Call expression that applies the lambda to arguments 2, 3, and 4
    const callMultExpr = new List([lambdaMultExpr, new LNumber(2), new LNumber(3), new LNumber(4)]);
  ...
  test('evaluate let expression', () => {
    const letExpr = new List([
      new LSymbol('let'), 
      new List([new List([new LSymbol('x'), new LNumber(5)])]), 
  ...
  test('evaluate let expression with multiple variables and arithmetic operations', () => {
    // Test with addition
    const letAddExpr = new List([
      new LSymbol('let'),
      new List([
        new List([new LSymbol('x'), new LNumber(5)]),
        new List([new LSymbol('y'), new LNumber(3)])
      ]),
      new List([new LSymbol('+'), new LSymbol('x'), new LSymbol('y')])
    ]);
    const resultAdd = evaluate(letAddExpr, env);
    expect(resultAdd.value).toEqual(8);

    // Test with subtraction
    const letSubExpr = new List([
      new LSymbol('let'),
      new List([
        new List([new LSymbol('x'), new LNumber(10)]),
        new List([new LSymbol('y'), new LNumber(4)])
      ]),
      new List([new LSymbol('-'), new LSymbol('x'), new LSymbol('y')])
    ]);
    ...
    const letMultExpr = new List([
      new LSymbol('let'),
      new List([
        new List([new LSymbol('x'), new LNumber(7)]),
        new List([new LSymbol('y'), new LNumber(6)])
      ]),
      new List([new LSymbol('*'), new LSymbol('x'), new LSymbol('y')])
    ]);
    ...
    // Test with division
    const letDivExpr = new List([
      new LSymbol('let'),
      new List([
        new List([new LSymbol('x'), new LNumber(20)]),
        new List([new LSymbol('y'), new LNumber(4)])
      ]),
      new List([new LSymbol('/'), new LSymbol('x'), new LSymbol('y')])
    ]);
